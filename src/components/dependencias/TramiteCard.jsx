...
const TramiteCard = ({ tramite, isExpanded, toggleExpand }) => {
      ...
      if (nivelLimitado >= valorEstrella) {
        ...
        estrellas.push(
          <div key={i} className="relative w-4 h-4">
            ...
            <svg className="absolute w-4 h-4 text-gray-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md overflow-hidden mb-4 border-l-4 ${esTramitePublicado ? 'border-green-500' : 'border-blue-300'} transition-all duration-300 hover:shadow-lg max-w-full`}>
      <div className="p-3">
        <div className="flex flex-col">
          <div className="flex justify-between items-start mb-2">
            ...
            <button
              ...
              className={`px-2 py-1 rounded-md text-xs font-medium flex items-center transition-colors ${isExpanded ? 'bg-gray-200 text-gray-700' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}
            >
              {isExpanded ? 'Ocultar' : 'Ver'}
              <svg className={`w-3 h-3 ml-1 transform transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </button>
          </div>
          
          <h3 className="text-base font-bold mb-1 text-gray-800 line-clamp-2">{tramite.tramite}</h3>
          
          <p className="text-xs font-medium text-gray-600 mb-2 flex items-center">
            <svg className="w-3 h-3 mr-1 text-blue-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className="truncate">{tramite.secretaria?.nombre || 'Secretaría no especificada'}</span>
          </p>
          
          <div className="mb-2">
            <p className="text-xs font-semibold text-gray-700 mb-1">Nivel de digitalización:</p>
